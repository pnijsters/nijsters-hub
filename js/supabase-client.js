...
// Admin: set a member's first/last name. The RPC validates + is authoritative;
// empty strings are stored as NULL.
export async function setMemberName(userId, firstName, lastName) {
  const { error } = await supabase.rpc('set_member_name', {
    p_user_id: userId, p_first_name: firstName, p_last_name: lastName,
  })
  if (error) throw new Error(`set_member_name failed: ${error.message}`)
}
