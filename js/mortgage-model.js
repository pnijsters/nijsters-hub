...
export function overlayActuals(terms, statements, base) {
  ...
  let aheadOfSchedule = 0, monthsAhead = null, extraPace = 0
  if (latest && latest.unpaid_principal != null) {
    ...
    // Observed overpayment pace ($/month): the cumulative dollars paid beyond
    // the contract, spread over the payments made. Folds in both explicit extra
    // principal and the biweekly 13th-payment effect. Feeds the "on this pace"
    // forward projection so it continues reality instead of dropping to
    // contractual-only.
    if (aheadOfSchedule > 0 && elapsed > 0) {
      extraPace = Math.round((aheadOfSchedule / elapsed) * 100) / 100
    }
  }
  ...
  return {
    ...
    aheadOfSchedule, monthsAhead, extraPace, latest, project, projectOpts,
  }
}
